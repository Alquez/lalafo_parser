...
""" Данный код используется для сбора данных с сайта lalafo.kg, с использованием Selenium, но при использовании 
браузера Chrome, данные собираются, но на определенном объекте(блок с рекламой или объявлением) останавливается, и сайт
отбрасывает назад на определенный объект, т.е. данные собираются, но на определенном блоке останавливается. 
Поэтому практичнее использовать API, который позволяет собирать данные сразу на всех страницах сайта. 
Для корректной работы кода необходимо чтобы сработала первая часть кода, 
затем раскомментировать вторую часть кода и закомментировать первую."""
